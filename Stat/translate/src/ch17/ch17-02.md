...
`SD` và `SE` khác nhau.[^3] `SD` áp dụng cho việc trải rộng trong danh sách các số. Nó được thực hiện bằng phương pháp được giải thích trên trang. 71. Ngược lại, `SE` áp dụng cho khả năng thay đổi cơ hội - ví dụ: trong tổng các lần rút.
...
> Tổng các lần rút rất có thể vào quanh......, cho hoặc nhận ......hoặc tương tự. `expected value` của tổng sẽ điền vào chỗ trống đầu tiên. `SE` của tổng điền vào chỗ trống thứ hai.
